...
import subprocess  # Для запуска внешнего скрипта (serverftp.py)
...
def receivePhoto():
    """Функция для приема фотографии, вызывается при нажатии кнопки."""
    try:
        # Запуск внешнего скрипта serverftp.py для приема фотографии
...
# Кнопка для получения фотографии
