...
from tkinter import filedialog  # Для открытия диалога выбора файла
...
class ClientSocket:
    ...
    def write(self):
        message = self.input.get("1.0", "end").strip()  # Убираем лишние пробелы и переносы строк
        if message:  # Проверяем, что сообщение не пустое
            self.client.send(message.encode("utf-8"))
            self.input.delete("1.0", "end")

    def send_image(self):
        """Открывает диалог для выбора изображения и отправляет его на сервер"""
